            ...
            uniform float4 SourceSize;    
             ...
             float4 GetSource(float2 uv, float2 offset = 0.0, float mipLevel = 0.0) {
              ...
              float4 minDepth= float4(
                GetSource(input.texcoord, float2(0, 1), HiZBufferFromMipLevel).r,
                GetSource(input.texcoord, float2(1, 0), HiZBufferFromMipLevel).r,
                GetSource(input.texcoord, float2(0, 0), HiZBufferFromMipLevel).r,
