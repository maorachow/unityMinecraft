...
SAMPLER(sampler_ContactShadowMap_LinearClamp);
...
{
#if _CONTACT_SHADOW
    return SAMPLE_TEXTURE2D(_ContactShadowMap, sampler_ContactShadowMap_LinearClamp, normalizedScreenSpaceUV).r;
#else
    return 1.0;
#endif
}

half MainLightRealtimeShadowWithContactShadow(float4 shadowCoord,float2 normalizedScreenSpaceUV)
{
    #if !defined(MAIN_LIGHT_CALCULATE_SHADOWS)
        return half(1.0);
    #elif defined(_MAIN_LIGHT_SHADOWS_SCREEN) && !defined(_SURFACE_TYPE_TRANSPARENT)
        float contactShadow= GetContactShadow(normalizedScreenSpaceUV);
        return min(SampleScreenSpaceShadowmap(shadowCoord),contactShadow);
    #else
...
{
    half realtimeShadow = MainLightRealtimeShadowWithContactShadow(shadowCoord,normalizedScreenSpaceUV);
